...
def genericRule (port=None, src_ip=None, dst_ip=None, transport_protocol=None, ip_protocol=IPv4_CONFIG):
    ...
    Args:
        port (int, optional): block the traffic from specific port . Defaults to None.
        src_ip (IPAddr, optional): block the traffic from specific host recongnized by the ip. Defaults to None.
        dst_ip (IPAddr, optional): block the traffic to specific host recongnized by the ip. Defaults to None.
        transport_protocol (pkt.ipv4.UDP_PROTOCOL || pkt.ipv4.TCP_PROTOCOL, optional): block the traffic with some transport protocol (UDP or TCP). Defaults to None.
        ip_protocol (pkt.ethernet.IP_TYPE || pkt.ethernet.IPV6_TYPE, optional): ip version recognized for the hosts. Defaults to IPv4_CONFIG.
...
class Firewall (EventMixin):
    ...
    def loadRules(self, event):
        # Install the rules on the switch
    ...
    def setRules (self, filename):
        # Id of the switch where the firewall will be installed
        self.swith_id = MAIN_SWITCH_ID 
        # Read the rules from the config file (json) and store them 
